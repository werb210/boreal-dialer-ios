...
const hoisted = vi.hoisted(() => ({
  get: vi.fn(async () => ({ data: { success: true, data: { token: "token-abc-12345", ttl: 60 } } }))
}));
...
describe("twilioTokenService", () => {
  ...
  it("fetches a voice token from API envelope", async () => {
    await expect(fetchVoiceToken()).resolves.toBe("token-abc-12345");
    expect(hoisted.get).toHaveBeenCalledWith(TELEPHONY_TOKEN_ENDPOINT);
  });

  it("caches token responses", async () => {
    await expect(getVoiceToken()).resolves.toBe("token-abc-12345");
    await expect(getVoiceToken()).resolves.toBe("token-abc-12345");
