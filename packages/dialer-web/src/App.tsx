...
export default function App() {
  ...
  useEffect(() => {
    initializeDialerAuthState()
      .then((state) => setAuthed(Boolean(state.token)))
  ...
  if (!ready) return <div className="bd-section">Loading...</div>;

  return <><IncomingCallOverlay />{authed ? <RootTabs onLogout={() => setAuthed(false)} /> : <LoginGate onAuthed={() => setAuthed(true)} />}</>;
}
